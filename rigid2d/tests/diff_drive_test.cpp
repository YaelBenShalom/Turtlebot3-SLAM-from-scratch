...
TEST_CASE( "Test DiffDrive constructors" ) {
    ...
    REQUIRE( rigid2d::almost_equal(pose.x, rigid2d::PI/2 * wheel_radious, 1e-3));
    ...
    wheel_vel = diff_drive.updateOdometryWithAngles(right_angle, left_angle);
    wheel_angle = diff_drive.get_wheel_angle();
    REQUIRE( rigid2d::almost_equal(wheel_angle.right_wheel_angle, rigid2d::normalize_angle(right_angle)));
    REQUIRE( rigid2d::almost_equal(wheel_angle.left_wheel_angle,  rigid2d::normalize_angle(left_angle)));
